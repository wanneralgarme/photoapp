...
function ImageGridArchitecture() {
  const [page, setPage] = useState(1); // Initialize page state

  const fetchImages = async ({ queryKey }) => {
    const { data } = await axios.get(`http://localhost:8888/${queryKey[0]}?category=architecture&page=${queryKey[1]}`);
    return data;
  };
...
export default ImageGridArchitecture;
